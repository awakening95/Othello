...
| 종류    | 명령         | 의미 |
  ------- | -------     | -------
| Request | [TURN]1 1   | 상대 플레이어가 1행 1열에 돌을 둔 것을 알리고 플레이어에게 돌을 놓으라고 요청한다. 게임을 시작할 때 가장 먼저 돌을 놓는 플레이어에게는 DATA를 포함하지 않고 [TRUN]만 보낸다. |
...
| Notify  | [EXIT]      | 상대 플레이어가 종료한 것을 알린다.(알수없는 오류로 인해 종료할 때도 포함) |
